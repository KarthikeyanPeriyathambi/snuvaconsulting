...
const JobDetailScreen = () => {
  ...
  const applyHandler = () => {
    if (!userInfo) {
      ...
      navigate(`/chatbot/${resume.id}/${jobId}`);
    } else {
          ...
          <div className="lg:col-span-2">
            <div className="bg-white rounded-lg shadow-md overflow-hidden">
              <div className="p-6">
                ...
                {job.chatbotQuestions && job.chatbotQuestions.length > 0 && (
                  <div className="mb-8">
                    <h2 className="text-xl font-semibold mb-3">Screening Questions</h2>
                    <ul className="list-disc pl-5 space-y-2 text-gray-700">
                      {job.chatbotQuestions.map((req, index) => (
                        <li key={index}>{req}</li>
                      ))}
                    </ul>
                  </div>
                ...
                {job.extraDetails && job.extraDetails.compliance && job.extraDetails.compliance.length > 0 && (
                  <div className="mb-8">
                      ...
                      <table className="min-w-full divide-y divide-gray-200">
                        <thead className="bg-gray-50">
                          <tr>
                            ...
                            <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Due Type</th>
                          </tr>
                        </thead>
                        <tbody className="bg-white divide-y divide-gray-200">
                          {job.extraDetails.compliance.map((comp) => (
                            <tr key={comp.compliance_id} className="hover:bg-gray-50 transition-colors">
                              <td className="px-6 py-4 whitespace-normal text-sm font-medium text-gray-900">{comp.item_name}</td>
                              <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{comp.group_name}</td>
                              <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{comp.due_type}</td>
                            </tr>
                          ))}
                        </tbody>
                      </table>
